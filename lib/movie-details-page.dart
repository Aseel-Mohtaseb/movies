...
import 'package:movies_project/database-helper.dart';
import 'package:movies_project/movie-details.dart';
...
class MovieDetailsPageState extends State<MovieDetailsPage> {
  ...
  Future<MovieDetails> fetchMovieDetails({required String movieId}) async {
    print(movieId);
    http.Response response= await http.get(Uri.parse("https://www.omdbapi.com/?i=$movieId&apikey=5f5d22fd"));
  ...
  Future<String> fetchFullPlot({required String movieId}) async {
    http.Response plotResponse= await http.get(Uri.parse("https://www.omdbapi.com/?i=$movieId&plot=full&apikey=5f5d22fd"));
                  ...
                  child: Column(
                    children: [
                      Row(
                        ...
                        children: [
                          Flexible(
                              child: Text(movieDetails!.title, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 30,),),
                          ),
                          Container(
                            alignment: Alignment.centerRight,
                            child: IconButton(
                              onPressed: () async {
                                await DatabaseProvider.db.isExist(movieDetails.imdbID
                                ).then((value) {
                                  print(value); exists=value;
                                });
                                print(exists);
                                if(exists){
                                  DatabaseProvider.db.removeMovie(movieDetails.imdbID);
                                }
                                else{
                      ...
                      Row(
                        ...
                        children: [
                          if(movieDetails.image == 'N/A')
                            Image.asset('assets/images/default-image.jpg',width: 180, height: 250,)
                          else
                            Image.network(movieDetails.image, width: 180, height: 250,),
                          Flexible(
                              child: Column(
                                  ...
                                  children: [
                                    ...
                                    SizedBox(height: 8,),
                                    Text("Writer: " + movieDetails.writer),
                                    SizedBox(height: 8,),
                      ...
                      GestureDetector(
                        ...
                        child:Text(displayFullPlot ? movieDetails.plot : movieDetails.plot + "\n\nclick to see the full plot"),
